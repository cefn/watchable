 ...
 *
 * The `ProxyPromise<T>` attaches handlers to the original `Promise<T>` `.then()`
 * and `.catch()` just once. Promises derived from it use a subscription- (and
 * unsubscription-) based mechanism that monitors these handlers.
 *
 * Every time you call `.subscribe()`, `.then()` `.catch()` or `.finally()` on a `ProxyPromise<T>` the
 * returned Promise has an additional `unsubscribe()` method allowing it to be
 * cleanly detached from the upstream Promise without creating memory leaks.
 *
 ...
 * any given `Promise<T>` facilitating arbitrary async/await patterns. Behind the
 * scenes, `resolve` is implemented simply as
